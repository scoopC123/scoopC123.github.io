...
function loadContent() {
  ...
  if (localStorage.getItem("covidJsObj") === null || (currentDate - storedDate)/millisperday >= 1){
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 
        ...
        var d1 = new Date();
		localStorage.setItem("storedDate", d1.getTime());
		storedDate = parseInt(localStorage.getItem('storedDate'));
	  ...
	  for (let e of covidJsObj.Countries){
		 if (e.TotalDeaths >= 50000) {
		newArray.push({
			"Slug":e.Slug,
			"TotalConfirmed": e.TotalConfirmed,
			"TotalDeaths": e.TotalDeaths,
			"Population": populations[e.Slug],
			"TotalConfirmedPer100000": Math.round((e.TotalConfirmed/populations[e.Slug]) * 100000),
			})	
		 }
		};
	  ...
	  //Q1
        = (dayjs().format("YYYY-MM-DD")) + " " + "Covid 19 Hotspots" ;
  ...
  xhttp.send();
	  ...
	  storedJson = localStorage.getItem('covidJsObj');
	  covidJsObj = JSON.parse(storedJson);
